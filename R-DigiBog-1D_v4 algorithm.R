...
# cohorts
layer_mass <- rep(NA,t_extent); layer_mass[1] <- prod(0,10) # g cm-2, gives an initial peat mass to start the model
...
daily_temp_output <- daily_precip_output <- vector()
...
for (i in 1:t_extent){
  ...
  t_and_pet <- generate_daily_temp_pet(temp_12mn,temp_12mn_sd); precip <- generate_daily_precip(precip_12mn,precip_12mn_sd) # grab climate sampling
  daily_t <- t_and_pet[1:365] # degree C
  daily_pet <- t_and_pet[366:730]/10 # cm
  daily_p <- precip/10 # cm
  # daily step
  repeat{
    ...
    if (daily_t[time_counter] > 0){
      ...
      # wt and pond
      wt_change_flux <- timestep*dHdt(daily_p[time_counter]*365,daily_pet[time_counter]*aet_pet_ratio(peat_height-wt_height,10,0.4)*365,
                             snowmelt*365,porosity,peat_transmissivity,wt_height,lateral_extent,drain_effi)*porosity # cm, wt change divided by porosity
      ...
      if (wt_height <= 0) {print("wt_height warning"); wt_height <- 0.0000001} # force no negative wt
      ...
      # production
      daily_prod <- c(daily_prod,prod(wtd,daily_t[time_counter])/365) # g cm-2 day-1
    } else {
      daily_decay <- c(daily_decay,0) # no decay
      snow <- snow+daily_p[time_counter] # snow accumulation
      snowmelt <- 0 # no snowmelt
      wtd <- peat_height-wt_height-pond # cm, daily wtd
      daily_prod <- c(daily_prod,0) # no production
    }
